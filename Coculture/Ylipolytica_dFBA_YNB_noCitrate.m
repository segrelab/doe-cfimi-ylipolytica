...
endGrowth_idx = find(flux{1}(1:end,bio_idx)==0,1);
...
fill(ax,t,[0.01,0.01,1,1],0.7.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
semilogy(ax,time,1E3.*biomass{1},'-', 'LineWidth',lineWidth, 'Color',Cfimi_color); hold(ax,'off');
xlim(ax,[0, time(end)])
ax.YScale = 'log'; ylim(ax,[0, 1])
...
fill(ax,t,[0,0,9,9],0.7.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
...
fill(ax_c,t,[0,0,5E-3,5E-3],0.7.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax_c,'on');
...
fill(ax,t,[-1,-1,8,8],0.7.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
for ii = 1:numel(col)
...
fill(ax,t,[0,0,12,12],0.7.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
...
fill(ax,t,[-5,-5,1,1],0.7.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
for ii = 1:numel(col)
    h(ii) = plot(ax,time,exchFlux_mean{1}(:,col(ii)), 'LineWidth',lineWidth, 'Color',c(ii,:));
end; hold(ax,'off');
xlim(ax,[0, time(end)])
...
fill(ax,t,[0,0,60,60],0.7.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
for ii = 1:numel(idx)
...
fill(ax_mM,t,[0,0,0.2,0.2],0.7.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25);
for ii = 1:numel(idx_mM)
    h(ii) = plot(ax_mM,time,exchMets_mM(:,met_idx(idx_mM(ii))), 'LineWidth',lineWidth, 'Color',c(idx_mM(ii),:)); hold(ax_mM,'on');
end; hold(ax_mM,'off');
...
xlim(ax_uM,[0, time(end)]); ylim(ax_mM,[0,0.05]); ylim(ax_uM,[0,0.2])
