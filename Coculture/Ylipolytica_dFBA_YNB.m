...
fill(ax,t,[0.01,0.01,10,10],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
semilogy(ax,time,1E3.*biomass{1},'-', 'LineWidth',lineWidth, 'Color',Cfimi_color); hold(ax,'off');
xlim(ax,[0, time(end)])
ax.YScale = 'log'; ylim(ax,[0, 10])
...
fill(ax,t,[-1,-1,8,8],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
for ii = 1:numel(col)
...
fill(ax,t,[0,0,12,12],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
...
fill(ax,t,[-5,-5,1,1],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
for ii = 1:numel(col)
    h(ii) = plot(ax,time,exchFlux_mean{1}(:,col(ii)), 'LineWidth',lineWidth, 'Color',c(ii,:));
end; hold(ax,'off');
xlim(ax,[0, time(end)])
...
fill(ax_mM,t,[0,0,0.2,0.2],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax_mM,'on');
