...
for numModel = 1:length(model)
    ...
    exchFlux_rxns{numModel}(rm_idx) = []; exchFlux{numModel}(:,rm_idx) = [];
...
fill(ax,t,[0.01,0.01,3,3],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
semilogy(ax,time,1E3.*biomass{1},'-', 'LineWidth',lineWidth, 'Color',Cfimi_color); hold(ax,'off');
xlim(ax,[0, time(end)])
ax.YScale = 'log'; ylim(ax,[0, 3])
...
fill(ax,t,[0,0,4,4],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
...
% Positive Flux
[~,col] = find(exchFlux{1} > 0); col = unique(col);
c = cbrewer('qual','Set1',numel(col));
n = 4;
if ishandle(n); clf(findobj('Type','Figure', 'Number',n)); end
fig = figure(n); fig.Name = 'Positive Flux'; ax = axes(fig); clear h
fill(ax,t,[-10,-10,50,50],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
for ii = 1:numel(col)
    h(ii) = plot(ax,time,exchFlux{1}(:,col(ii)), 'LineWidth',lineWidth, 'Color',c(ii,:));
end; hold(ax,'off');
xlim(ax,[0, time(end)])
grid(ax,'on'); ax.FontSize = axesLabelSize;
xlabel(ax, 'Time [hours]', 'FontSize',xyLabelSize)
...
fill(ax,t,[0,0,0.25,0.25],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
...
fill(ax,t,[-10,-10,15,15],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
for ii = 1:numel(col)
    h(ii) = plot(ax,time,exchFlux{1}(:,col(ii)), 'LineWidth',lineWidth, 'Color',c(ii,:));
end; hold(ax,'off');
xlim(ax,[0, time(end)])
...
fill(ax,t,[0,0,70,70],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax,'on');
for ii = 1:numel(idx)
...
fill(ax_mM,t,[0,0,0.05,0.05],0.5.*[1,1,1], 'LineStyle','none', 'FaceAlpha',0.25); hold(ax_mM,'on');
...
xlim(ax_uM,[0, time(end)]); ylim(ax_mM,[0,0.05]); ylim(ax_uM,[0,0.1])
